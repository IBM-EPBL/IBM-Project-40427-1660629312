...
def image_database_retrieval():
    ...
    for i in image_result_retrieved:
        if (i['doc']['email'] in image_result.keys()):
            ...
            if (n > o):
                image_result[i['doc']['email']] = {'name': i['doc']['name'], 'image': i['doc']['image'],
                                                   'date': i['doc']['datetime']}
        else:
            image_result[i['doc']['email']] = {'name': i['doc']['name'], 'image': i['doc']['image'],
                                               'date': i['doc']['datetime']}
...
def forgotpassword():
    ...
    if (flask.request.method == 'POST'):
        ...
        if (reset_email in data.keys()):
            ...
            # k.append(token)
            # print(token)
            send_mail(reset_email, "password reset",
                      f"Reset password URL is {flask.url_for('resetpassword', token=token, _external=True)}")
        else:
            print('#########################')
            pass
...
def resetpassword(token):
    ...
    if flask.request.method == "GET":
        ...
        token1 = token1.split(',')
        print(token1)
        generated_date = datetime(int(token1[1]), int(token1[2]), int(token1[3]), int(token1[4]), int(token1[5]),
                                  int(token1[6]), int(token1[7]))
...
def prediction():
    ...
    if (flask.request.method == 'POST'):
        ...
        try:
            os.remove('static\imagedata\6782.png')
        except:
            pass
        ...
        img_retrive.save('static\imagedata\6791.png')
